...
export async function POST(req: Request) {
  ...
  const { nombre_contacto, telefono_contacto, direccion_contacto, marca, items } = body;
  ...
  const { data: pedido, error: errorPedido } = await supabase
    ...
    .insert({
      origen: 'catalogo_web',
      marca: marca === 'via_vana' ? 'via_vana' : 'grido',
      estado: 'nuevo',
      nombre_contacto,
      telefono_contacto,
      direccion_contacto,
  ...
  await supabase.from('log_auditoria').insert({
    ...
    detalle: `Pedido #${pedido.numero} desde el catálogo (${marca === 'via_vana' ? 'Via Vana' : 'Grido'}), total $${total.toFixed(2)}`,
  });
