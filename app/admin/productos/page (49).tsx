...
function FormularioProducto({
  ...
  async function guardar(e: React.FormEvent) {
    ...
    if (producto) {
      ...
      await registrarLog(
        ...
        'producto',
        producto.id,
        `Producto "${form.nombre}" editado (precio: $${form.precio_normal})`
      );
    } else {
      const { data: creado } = await supabase.from('productos').insert(form).select().single();
      await registrarLog(
        'crear_producto',
        'producto',
        creado?.id ?? null,
        `Producto "${form.nombre}" creado (precio: $${form.precio_normal})`
      );
    }
    setGuardando(false);
    onGuardado();
  ...
  async function eliminar() {
    ...
    await registrarLog('eliminar_producto', 'producto', producto.id, `Producto "${producto.nombre}" eliminado`);
    onGuardado();
  }
