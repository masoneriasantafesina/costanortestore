...
export default function UsuariosPage() {
  ...
  async function cambiarRol(perfil: Perfil, rol: Perfil['rol']) {
    ...
    await registrarLog(
      ...
      'usuario',
      perfil.id,
      `Rol de ${perfil.nombre || perfil.email} cambiado de "${perfil.rol}" a "${rol}"`
    );
    await cargar();
  }
  ...
  async function eliminarUsuario(perfil: Perfil) {
    ...
    await registrarLog(
      ...
      'usuario',
      perfil.id,
      `Acceso eliminado para ${perfil.nombre || perfil.email}`
    );
    await cargar();
  }
          ...
          <div key={p.id} className="flex items-center justify-between px-4 py-3">
            <div>
              <p className="text-sm font-medium text-frost-800 flex items-center gap-2">
                {p.nombre || p.email}
                {p.rol === 'nuevo' && (
                  <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-card bg-yellow-100 text-yellow-700">
...
function FormularioInvitar({ onCerrar, onInvitado }: { onCerrar: () => void; onInvitado: () => void }) {
  ...
  const [rol, setRol] = useState<'administrador' | 'operador' | 'nuevo'>('operador');
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <form onSubmit={invitar} className="relative bg-white rounded-card p-6 w-full max-w-sm space-y-3">
        ...
        <select
          className="input-field"
          value={rol}
          onChange={(e) => setRol(e.target.value as 'administrador' | 'operador' | 'nuevo')}
        >
          <option value="nuevo">Nuevo (sin acceso)</option>
          <option value="operador">Operador</option>
          <option value="administrador">Administrador</option>
