...
import type { Cliente, UnidadNegocio } from '@/lib/types';
...
export default function ClientesPage() {
  ...
  const [filtroUnidad, setFiltroUnidad] = useState<UnidadNegocio | 'todas'>('todas');
  ...
  const filtrados = clientes
    .filter((c) => filtroUnidad === 'todas' || c.unidad_negocio === filtroUnidad)
    .filter((c) => `${c.nombre_apellido} ${c.nombre_comercio}`.toLowerCase().includes(busqueda.toLowerCase()));
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {(Object.entries(UNIDAD_NEGOCIO_LABEL) as [UnidadNegocio, string][]).map(([valor, etiqueta]) => (
            <button
              key={valor}
              onClick={() => setFiltroUnidad(valor)}
              className={`px-3 py-2 rounded-card text-sm font-medium ${
                filtroUnidad === valor
                  ? 'bg-frost-700 text-white'
                  : 'bg-white text-frost-700 border border-frost-200'
