    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative bg-white rounded-card p-6 w-full max-w-lg max-h-[85vh] overflow-y-auto">
        ...
        <div className="flex gap-2">
          <button onClick={onVerHistorial} className="flex-1 text-sm font-medium text-frost-700 border border-frost-200 rounded-card px-3 py-2 hover:bg-frost-50">
...
function HistorialPedidosCliente({ cliente, onCerrar }: { cliente: Cliente; onCerrar: () => void }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative bg-white rounded-card p-6 w-full max-w-lg max-h-[85vh] overflow-y-auto">
        ...
        <div className="space-y-2">
          {pedidos?.map((p) => (
            <div key={p.id} className="bg-frost-50 rounded-card p-3">
              <div className="flex items-center justify-between mb-1">
                <p className="text-sm font-medium text-frost-800">
                  #{p.numero} · {new Date(p.creado_en).toLocaleDateString('es-AR')}
                </p>
                <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-card ${colorEstado(p.estado)}`}>
                  {ESTADOS_LABEL[p.estado]}
                </span>
              </div>
              <div className="text-xs text-frost-500 space-y-0.5 mb-1">
              ...
              <div className="flex justify-between text-sm font-semibold text-frost-800 border-t border-frost-100 pt-1 mt-1">
                <span>
                  {p.metodo_pago
                    ? `${p.metodo_pago === 'cuenta_corriente' ? 'Cuenta corriente' : p.metodo_pago}${
                        p.metodo_pago === 'cuenta_corriente' ? (p.pagado ? ' (pagado)' : ' (pendiente)') : ''
                      }`
                    : 'Sin cobro definido'}
                </span>
