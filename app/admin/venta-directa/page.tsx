...
import { UNIDAD_NEGOCIO_LABEL, UNIDAD_NEGOCIO_COLOR, MARCA_PRODUCTO_LABEL, marcaDeUnidad } from '@/lib/types';
...
export default function VentaDirectaPage() {
  ...
  useEffect(() => {
    ...
    const marca = marcaDeUnidad(clienteSeleccionado.unidad_negocio);
    supabase
      .from('productos')
      .select('*')
      .eq('marca', marca)
  ...
  async function confirmarPedido() {
    ...
    const { data: pedido, error: errorPedido } = await supabase
      ...
      .insert({
        origen: 'carga_manual',
        marca: marcaDeUnidad(clienteSeleccionado.unidad_negocio),
        estado: 'nuevo',
    ...
    await registrarLog(
      ...
      'pedido',
      pedido.id,
      `Pedido #${pedido.numero} cargado por Venta Directa para ${clienteSeleccionado.nombre_apellido}, total $${total.toFixed(2)}`
    );
          ...
          <div className="bg-white rounded-card border border-frost-100 p-4 mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-frost-500">
                ...
                {MARCA_PRODUCTO_LABEL[marcaDeUnidad(clienteSeleccionado.unidad_negocio)]}
              </p>
            </div>
            ...
            <div className="lg:col-span-2">
              ...
              {!cargandoProductos && productosFiltrados.length === 0 && (
                <p className="text-frost-500 text-sm">
                  No hay productos de marca {MARCA_PRODUCTO_LABEL[marcaDeUnidad(clienteSeleccionado.unidad_negocio)]} cargados
                  todavía.
                </p>
              )}
