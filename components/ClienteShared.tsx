...
import { CONDICION_IVA_LABEL, ESTADOS_LABEL, UNIDAD_NEGOCIO_LABEL, UNIDAD_NEGOCIO_COLOR } from '@/lib/types';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative bg-white rounded-card p-6 w-full max-w-lg max-h-[85vh] overflow-y-auto">
        ...
        <div className="flex gap-2 mb-4 flex-wrap">
          <span
            className={`inline-block text-xs font-semibold px-2 py-1 rounded-card ${UNIDAD_NEGOCIO_COLOR[cliente.unidad_negocio]}`}
          >
            {UNIDAD_NEGOCIO_LABEL[cliente.unidad_negocio]}
          </span>
          ...
          {cliente.unidad_negocio === 'grido_store' && (
            <span
        ...
        <div className="space-y-2 text-sm text-frost-700 mb-4">
          ...
          {cliente.unidad_negocio === 'grido_store' && cliente.tipo_cliente === 'comodato_grido' && (
            <p>
...
export function FormularioCliente({
  ...
  async function guardar(e: React.FormEvent) {
    ...
    if (cliente) {
      ...
      await registrarLog(
        ...
        'cliente',
        cliente.id,
        `Cliente "${form.nombre_apellido}" editado`
      );
    } else {
      const { data: creado } = await supabase.from('clientes').insert(form).select().single();
      await registrarLog(
        'crear_cliente',
        'cliente',
        creado?.id ?? null,
        `Cliente "${form.nombre_apellido}" creado (${estadoNuevo === 'posible' ? 'posible cliente' : 'confirmado'})`
      );
    }
    setGuardando(false);
    onGuardado();
  ...
  async function eliminar() {
    ...
    await registrarLog('eliminar_cliente', 'cliente', cliente.id, `Cliente "${cliente.nombre_apellido}" eliminado`);
    onGuardado();
  }
  ...
  async function confirmarCliente() {
    ...
    await registrarLog(
      ...
      'cliente',
      cliente.id,
      `Cliente "${cliente.nombre_apellido}" pasó de posible a confirmado`
    );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      >
        ...
        <div>
          <label className="text-xs font-semibold text-frost-500 uppercase">Unidad de negocio</label>
          <select
            required
            className="input-field mt-1"
            value={form.unidad_negocio}
            onChange={(e) =>
              setForm({ ...form, unidad_negocio: e.target.value as Cliente['unidad_negocio'] })
            }
          >
            {Object.entries(UNIDAD_NEGOCIO_LABEL).map(([valor, etiqueta]) => (
        ...
        {form.unidad_negocio === 'grido_store' && (
          <>
            <div>
...
export function HistorialPedidosCliente({ cliente, onCerrar }: { cliente: Cliente; onCerrar: () => void }) {
  ...
  function etiquetaCobro(p: Pedido) {
    ...
    if (p.metodo_pago === 'pendiente_pago') return p.pagado ? 'Pendiente de pago (cobrado)' : 'Pendiente de pago';
